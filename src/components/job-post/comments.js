    ...
    handleCommentSketchesDisplay =  (commentSketches) => {
        ...
        if (commentSketches) {
            for (let fileKey in Object.keys(commentSketches)) {
                files.push(<span key={fileKey} style={{marginLeft: '10px', color: '#2a41e8', fontWeight: '600'}} class="uploadButton-file-name">{commentSketches[fileKey].name ? `(${Number(fileKey) + 1}). ${commentSketches[fileKey].name}` : `(${Number(fileKey) + 1}). ${commentSketches[fileKey]}`}</span>);
            }
        return files;
    } else {
       return <span style={{marginLeft: '10px'}} id="uploadButtonCommentsSketchFileName" class="uploadButton-file-name">Images that might be helpful to accompany the comment</span>
    }
}
                    ...
                    <form method="post" id="add-comment">
                        ...
                        <div class="col-xl-11 col-lg-11">
                            <div class="uploadButtonx">
                                <input onChange={this.handleCommentSketches} id="uploadButtonAestheticSketch" class="uploadButton-inputx" type="file" name="commentSketches[]" accept="image/*, application/pdf" multiple="multiple" />
